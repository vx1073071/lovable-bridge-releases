...
```bash
# Windows x64
bun build bridge/index.ts --compile --target=bun-windows-x64 -o dist/LovableBridge.exe
# macOS Apple Silicon
bun build bridge/index.ts --compile --target=bun-darwin-arm64 -o dist/LovableBridge-mac-arm64
# macOS Intel
bun build bridge/index.ts --compile --target=bun-darwin-x64  -o dist/LovableBridge-mac-x64
# Linux x64
bun build bridge/index.ts --compile --target=bun-linux-x64   -o dist/LovableBridge-linux-x64
```
